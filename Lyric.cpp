...
void CLyrics::DisposeLrc()
{
    ...
    m_is_mlrc = false;
    // 检测是否为 GPT 生成的 MLRC
    for (const auto& line : m_lyrics_str)
    {
        if (line.find(L"[ml:") != wstring::npos)
        {
            m_is_mlrc = true;
            break;
        }
...
void CLyrics::CombineMlrcLyric()
{
    for (size_t i = 0; i + 1 < m_lyrics.size();)
    {
        // 相同时间：仍按普通双语歌词处理
        if (m_lyrics[i].time_start_raw == m_lyrics[i + 1].time_start_raw)
        {
            m_lyrics[i].translate = m_lyrics[i + 1].text;
